...
function show_class(subject: string, code: string, itchid: string) {
  console.log("Show class called.");
  var ret1 = "";
  require("request")({
    url: 'http://127.0.0.1:5000/' + subject + '1',
    json: true
  }, async function (error: any, response: { statusCode: number; }, body: any) {
    ...
    if (!error && response.statusCode === 200) {
      ...
      stuffs.forEach((element: { catalog_number: string; subject: string; title: string; description: string; }) => {
        if (element.catalog_number === code) {
          ...
          if (String(currentDate.getSeconds()).length === 1) {
            ...
            ret1 = ret1.concat(currentDate.getSeconds().toString());
          }
  ...
  require("request")({
    ...
    if (!error && response.statusCode === 200) {
      ...
      stuffs.classes.forEach((element: { catalog_number: any; meetings: string | any[]; class_number: string; enrollment_cap: number; instructors: string | any[]; }) => {
...
function show_class(subject: string, code: string, semester: string, year: string, itchid: string) {
...
client.on('interactionCreate', async (interaction: { isChatInputCommand?: any; channelId?: any; options?: any; reply?: any; commandName?: any; }) => {
  ...
  if (commandName === 'class') {
    ...
    var itchid:string = interaction.channelId;
    ...
    var sem_bool:string = interaction.options.getString('semester');
    var year_bool:string = interaction.options.getString('year');
    ...
    var itchid:string = interaction.channelId;
    const class1:string[] = interaction.options.getString('class1').split(" ")
    const class2:string[] = (interaction.options.getString('class2')) ? interaction.options.getString('class2').split(" ") : [];
    const class3:string[] = (interaction.options.getString('class3')) ? interaction.options.getString('class3').split(" ") : [];
    ...
    if ((sem_bool || year_bool) && !(sem_bool && year_bool)) {
      ...
      var semester:string = interaction.options.getString('semester');
      var year:string = interaction.options.getString('year');
    ...
    let ret:string = "```\"/class\" for 1 or more classes \n\nSource Code: \
